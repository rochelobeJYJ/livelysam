(function () {
  ...
  const GRID_LAYOUT_INIT_MARKER_STORAGE_KEY = 'gridLayoutInitialized';
  ...
  const LAYOUT_RESTORE_PENDING_STORAGE_KEY = 'layoutRestorePending';
  const WIDGET_META = {
    clock: { title: '시계', icon: '⏰' },
  ...
  LS.App = {
    ...
    _seedLayoutPersistence() {
      ...
      const initialized = LS.Storage.get(GRID_LAYOUT_INIT_MARKER_STORAGE_KEY, false);
      const hasManualPersistedLayout = Boolean(
        persisted?.layout?.length
        && !this._isAutoGeneratedLayoutReason(persisted?.reason)
      );

      if (!initialized) {
        if (!hasManualPersistedLayout) {
          const widgetVisibility = LS.Config.getWidgetVisibility();
          const defaultLayout = this._cloneLayout(DEFAULT_LAYOUT);
          const defaultVisibleLayout = defaultLayout.filter((item) => widgetVisibility?.[item.id] !== false);
          this._persistLayoutState(defaultLayout, {
            reason: 'seed-default-layout',
            widgetVisibility
          });
          LS.Storage.set(GRID_LAYOUT_INIT_MARKER_STORAGE_KEY, true);
          this._lastSafeVisibleLayout = this._cloneLayout(defaultVisibleLayout);
          if (this.grid && defaultVisibleLayout.length) {
            this._restoreVisibleGridLayout(defaultVisibleLayout);
          }
          return;
        }
    ...
    async _mountVisibleWidget(widgetId) {
      switch (widgetId) {
        case 'clock':
          if (LS.ClockWidget?._interval) LS.ClockWidget._tick?.();
          else LS.ClockWidget?.init?.();
          return;
        case 'weather':
          if (LS.WeatherWidget?._updateInterval) {
            if (LS.WeatherWidget?._lastData) LS.WeatherWidget.render?.(LS.WeatherWidget._lastData);
            else await LS.WeatherWidget.update?.();
          } else {
    ...
    async _applyLayoutSnapshotToGrid(layout = [], widgetVisibility = LS.Config.getWidgetVisibility()) {
      if (!this.grid) return false;

      const normalizedLayout = Array.isArray(layout) && layout.length
        ? this._cloneLayout(layout)
        : this._cloneLayout(DEFAULT_LAYOUT);
      const nextVisibility = widgetVisibility && typeof widgetVisibility === 'object'
        ? { ...widgetVisibility }
        : LS.Config.getWidgetVisibility();
      const visibleLayout = normalizedLayout.filter((item) => nextVisibility?.[item.id] !== false);
      const existingElements = new Map(
        Array.from(document.querySelectorAll('.grid-stack-item[gs-id]')).map((el) => [el.getAttribute('gs-id') || '', el])
      );
      const nextVisibleIds = new Set(visibleLayout.map((item) => item.id));
      const addedWidgetIds = [];

      this._suppressGridChange = true;
      this.grid.batchUpdate?.();

      existingElements.forEach((el, widgetId) => {
        if (nextVisibleIds.has(widgetId)) return;
        this.grid.removeWidget?.(el, true, false);
      });

      visibleLayout.forEach((item) => {
        const existingEl = existingElements.get(item.id);
        const widgetOptions = {
          id: item.id,
          x: item.x,
          y: item.y,
        ...
        if (existingEl) {
          this.grid.update(existingEl, widgetOptions);
          return;
        }

        const nextEl = this._createGridItemElement(item);
        if (!nextEl) return;
        this.grid.addWidget(nextEl, widgetOptions);
        addedWidgetIds.push(item.id);
      });
      ...
      WIDGET_VISIBILITY_FIELDS.forEach(([widgetId]) => {
        this._syncWidgetVisibilityField(widgetId, nextVisibility[widgetId] !== false);
      });
      ...
      for (const widgetId of addedWidgetIds) {
        try {
          await this._mountVisibleWidget(widgetId);
        } catch (error) {
          console.warn(`[LivelySam] Failed to mount widget after layout apply: ${widgetId}`, error);
        }
      }
    ...
    _applyPendingLayoutRestore() {
      const snapshot = LS.Storage.get(LAYOUT_RESTORE_PENDING_STORAGE_KEY, null);
      if (!snapshot || !Array.isArray(snapshot.layout)) {
        return false;
      }

      const widgetVisibility = snapshot.widgetVisibility && typeof snapshot.widgetVisibility === 'object'
        ? { ...snapshot.widgetVisibility }
        : LS.Config.getWidgetVisibility();
      const visibleLayout = Array.isArray(snapshot.visibleLayout) && snapshot.visibleLayout.length
        ? this._cloneLayout(snapshot.visibleLayout)
        : this._cloneLayout(snapshot.layout.filter((item) => widgetVisibility?.[item.id] !== false));

      if (visibleLayout.length > 0) {
        this._restoreVisibleGridLayout(visibleLayout);
        this._lastSafeVisibleLayout = this._cloneLayout(visibleLayout);
      }
    ...
    _captureCurrentLayoutSnapshot(options = {}) {
      const baseLayout = Array.isArray(options.baseLayout) && options.baseLayout.length
        ? this._cloneLayout(options.baseLayout)
        : this._getNormalizedLayout();
      const visibleLayout = Array.isArray(options.visibleLayout)
        ? this._cloneLayout(options.visibleLayout)
        : this._captureCurrentVisibleGridLayout();
      const widgetVisibility = options.widgetVisibility && typeof options.widgetVisibility === 'object'
        ? { ...options.widgetVisibility }
        : LS.Config.getWidgetVisibility();

      return {
        layout: visibleLayout.length
          ? this._mergeVisibleLayoutIntoNormalized(visibleLayout, baseLayout)
          : this._cloneLayout(baseLayout),
        visibleLayout: this._cloneLayout(visibleLayout),
        widgetVisibility
      };
    },

    _captureLayoutEditOrigin() {
      const snapshot = this._captureCurrentLayoutSnapshot();
    ...
    _getLayoutEditOrigin() {
      ...
      return {
        ...
        visibleLayout: Array.isArray(snapshot.visibleLayout)
          ? this._cloneLayout(snapshot.visibleLayout)
          : [],
    ...
    async _restoreLayoutEditOrigin() {
      ...
      const confirmed = await LS.Helpers.confirmModal(
        '배치 원복',
        '배치 편집에 들어가기 직전의 위치, 크기, 위젯 표시 상태로 정확히 되돌립니다. 현재 편집 중인 변경은 취소됩니다.',
      ...
      const visibleLayout = Array.isArray(snapshot.visibleLayout) && snapshot.visibleLayout.length
        ? this._cloneLayout(snapshot.visibleLayout)
        : snapshot.layout.filter((item) => snapshot.widgetVisibility?.[item.id] !== false);
      this._lastSafeVisibleLayout = this._cloneLayout(visibleLayout);
      ...
      LS.Helpers.showToast('편집 시작 전 배치로 되돌렸습니다.', 'success', 2400);
    },
    ...
    _initGrid() {
      ...
      this._lastSafeVisibleLayout = this._cloneLayout(visibleLayout);
      this._applyPendingLayoutRestore();
      this._applyLayoutEditMode();
      this.grid.on('change', () => {
    ...
    async resetLayout() {
      ...
      await this._applyLayoutSnapshotToGrid(defaultLayout, widgetVisibility);
      LS.Helpers.showToast('기본 레이아웃으로 되돌렸습니다.', 'success', 2200);
    },
    ...
    _bindLayoutEditInteractions() {
      ...
      surfaceEl.addEventListener('click', (event) => {
        ...
        if (closeButton) {
          ...
          event.stopPropagation();
          void this._closeWidgetFromLayoutEdit(closeButton.dataset.widgetClose || '');
          return;
        }
    ...
    _closeWidgetFromLayoutEditLegacy(widgetId) {
    ...
    _showHiddenWidgetOnCanvasLegacy(widgetId) {
    ...
    async _closeWidgetFromLayoutEdit(widgetId) {
      if (!this._layoutEditMode || !widgetId) return;

      const nextVisibility = LS.Config.getWidgetVisibility();
      if (!Object.prototype.hasOwnProperty.call(nextVisibility, widgetId)) return;
      nextVisibility[widgetId] = false;

      const nextLayout = this.grid
        ? this._mergeVisibleLayoutIntoNormalized(this._captureCurrentVisibleGridLayout())
        : this._getNormalizedLayout();
      const meta = this._getWidgetMeta(widgetId);

      this._persistLayoutState(nextLayout, {
        reason: 'widget-hide',
        widgetVisibility: nextVisibility
      });
      ...
      this._layoutEditMode = true;
      this._applyLayoutEditMode();
      LS.Helpers.showToast(`${meta.title} 위젯을 숨기고 편집 화면에서 다시 불러올 수 있습니다.`, 'info', 2200);
    },

    async _showHiddenWidgetOnCanvas(widgetId) {
      if (!widgetId) return false;
      ...
      this._layoutEditMode = true;
      this._applyLayoutEditMode();
      LS.Helpers.showToast(`${meta.title} 위젯을 기본 위치로 다시 표시했습니다.`, 'success', 2200);
      return true;
    },
    ...
    async _openHiddenWidgetPicker() {
      ...
      if (!widgetId) return;
      await this._showHiddenWidgetOnCanvas(widgetId);
    },
    ...
    _initSettingsModal() {
      ...
      document.getElementById('quickstart-save-btn')?.addEventListener('click', () => this._saveSettings());
        document.getElementById('reset-layout-btn')?.addEventListener('click', () => { void this.resetLayout(); });
    ...
    _ensureFloatingActionDock() {
      ...
      if (!revertBtn.dataset.bound) {
        ...
        revertBtn.addEventListener('click', () => { void this._restoreLayoutEditOrigin(); });
      }
